...
public:
  League(Trainer *player, vector<Trainer*> gym_leaders): 
    player(player), gym_leaders(gym_leaders), defeated() { }
  ...
  void run_league() {
    for(int i = 0; i < gym_leaders.size(); ++i) {
      ...
      trainer_battle(current_gym_leader);
    }
  ...
  void trainer_battle(Trainer *current_gym_leader) {
    int num_wins = 0;
    for(int i = 0; i < 5; ++i) {
